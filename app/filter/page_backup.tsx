...
// // Update OPERATORS_BY_TYPE to remove date-based operators
...
// // Update checkCondition function to remove business days logic
...
//         rootKey.children.forEach((mainKey: FilterKey) => {
//             if (mainKey.keyType === 'main') {
//                 // Add main key conditions
//                 mainKey.conditions.forEach((condition: FilterCondition) => {
...
//                 mainKey.children.forEach((subKey: FilterKey) => {
//                     if (subKey.keyType === 'sub' && subKey.keyColumn) {
//                         // Add sub key conditions
//                         subKey.conditions.forEach((condition: FilterCondition) => {
//                             if (condition.column) { // Only add if column is selected
//                                 conditions.push({
...
//       // Build the filter payload with the new page
//       const payload = buildFilterPayload(filterKeys);
//       payload.page = newPage;

//       // Make a POST request with the current filter conditions
//       const response = await fetch('http://localhost:5000/api/filters/claims', {
//         method: 'POST',
//         headers: {
//           'Content-Type': 'application/json',
...
//         // Update all the state with the new data
//         setClaims(data.claims);
//         setPage(newPage);
//         setTotalRecords(data.pagination.total);
...
//   // Update handlePageSizeChange function
//   const handlePageSizeChange = async (newSize: number) => {
//     try {
//       setIsLoading(true);

//       // Build the filter payload with the new page size
//       const payload = buildFilterPayload(filterKeys);
//       payload.page = 1; // Reset to first page when changing page size
//       payload.limit = newSize;
...
//                         keyColumn: 'claim_id',
//                         conditions: conditions
//                             .filter((c: BackendFilterCondition) => c.key === 'Claim Id')
//                             .map((c: BackendFilterCondition, index: number) => ({
...
//                 const subKeyConditions = conditions.filter((c: BackendFilterCondition) => c.key.startsWith('Sub Key:'));
//                 if (subKeyConditions.length > 0) {
//                     const subKeyGroups = subKeyConditions.reduce((acc: Record<string, BackendFilterCondition[]>, c: BackendFilterCondition) => {
...
//                         return acc;
//                     }, {} as Record<string, BackendFilterCondition[]>);
...
//                             id: `subgroup${index + 1}`,
//                             keyType: 'sub' as const,
//                             keyColumn: keyColumn,
//                             conditions: (conditions as BackendFilterCondition[]).map((c, condIndex) => ({
...
//                     key={filter.id}
//                     value={filter.name}
//                     onSelect={(value) => loadSavedFilter(value)}
//                   >
//                     <Check
//                       className={cn(
//                         "mr-2 h-4 w-4",
