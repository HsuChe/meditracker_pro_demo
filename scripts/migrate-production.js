...
async function migrate() {
  try {
    ...
    // Create ingested_data table with all required columns for all environments
    await client.query(`
    ...
    // Add self-reference for parent_ingestion_id
    await client.query(`
      ALTER TABLE ingested_data 
      ADD CONSTRAINT ingested_data_parent_ingestion_id_fkey 
      FOREIGN KEY (parent_ingestion_id) 
      REFERENCES ingested_data(ingested_data_id);
    `);
    console.log('Added parent_ingestion_id foreign key constraint');
    ...
    await client.query(`
      CREATE TABLE lut_entries (
        ...
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        ingestion_id INTEGER REFERENCES ingested_data(ingested_data_id),
        value TEXT,
    ...
    // Create indexes for ingested_data
    await client.query('CREATE INDEX idx_ingested_data_type ON ingested_data(type);');
    await client.query('CREATE INDEX idx_ingested_data_parent_id ON ingested_data(parent_ingestion_id);');
    await client.query('CREATE INDEX idx_ingested_data_activity_status ON ingested_data(activity_status);');
    console.log('Created indexes for ingested_data');
